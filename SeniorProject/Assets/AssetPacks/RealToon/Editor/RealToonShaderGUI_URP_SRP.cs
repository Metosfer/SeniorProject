...
static string dots_lbs_cd_string = "DOTS|HR - Use Compute Deformation";
        ...
        //Close-Up Size (Perspective Adjustment) [158]
        "This will adjust the size of the object when the camera is closer.",

        //Close-Up Size Smooth Transition (Perspective Adjustment) [159]
        "How smooth the transition of the sizing.",

        //Close-Up Size Distance (Perspective Adjustment) [160]
            ...
            if (ShowUI == true)
            {
                ...
                if (_TRANSMODE.floatValue == 1)
                {
                    if (_N_F_CO.floatValue == 1)
                    {
                        ...
                        if (ShowCutout)
                        {
                            ...
                            materialEditor.ShaderProperty(_N_F_SCO, new GUIContent(_N_F_SCO.displayName, TOTIPS[153]));
                ...
                if (_N_F_TP.floatValue == 1)
                {
                    ...
                    if (Triplanar)
                    {
                        ...
                        materialEditor.ShaderProperty(_TriPlaTile, new GUIContent(_TriPlaTile.displayName, TOTIPS[154]));
                        materialEditor.ShaderProperty(_TriPlaBlend, new GUIContent(_TriPlaBlend.displayName, TOTIPS[155]));
                ...
                if (_N_F_PA.floatValue == 1)
                {
                    ...
                    if (ShowPerspecAdju)
                    {
                        ...
                        materialEditor.ShaderProperty(_PresAdju, new GUIContent(_PresAdju.displayName, TOTIPS[156]));
                        materialEditor.ShaderProperty(_ClipAdju, new GUIContent(_ClipAdju.displayName, TOTIPS[157]));

                        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);

                        materialEditor.ShaderProperty(_PASize, new GUIContent(_PASize.displayName, TOTIPS[158]));
                        materialEditor.ShaderProperty(_PASmooTrans, new GUIContent(_PASmooTrans.displayName, TOTIPS[159]));
                        materialEditor.ShaderProperty(_PADist, new GUIContent(_PADist.displayName, TOTIPS[160]));
